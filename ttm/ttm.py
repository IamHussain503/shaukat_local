...
class MusicGenerationService(AIModelService):
    ...
    def update_block(self):
        self.current_block = self.subtensor.block
        if self.current_block - self.last_updated_block > 30:
            ...
            bt.logging.info(f"Next update will be at block: {self.last_updated_block + 30}")
    ...
    def process_response(self, axon, response, prompt, api=False):
        try:
            ...
            if response is not None and isinstance(response, MusicGeneration) and response.music_output is not None and response.dendrite.status_code == 200:
                ...
                if api:
                    file = self.handle_music_output(axon, music_output, prompt, response.model_name, api=True)
                    return file
                else:
    ...
    def handle_music_output(self, axon, music_output, prompt, model_name, api=False):
        ...
        try:
            # Save the audio file based on whether it's for the API or regular process
            if api:
                file_name = f"api_sample_{axon.hotkey}.wav"
                output_path = os.path.join(api_output_dir, file_name)
            else:
